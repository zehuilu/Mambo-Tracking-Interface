...
while idx < 36:
    ...
    if msg_planner:
        if idx > -30:
            data_planner = json.loads(msg_planner.decode())
            flag_append = data_planner["flag_append"]
        
            #data_mocap = np.fromstring(msg_mocap, dtype=float)
            #data_mocap = data_mocap[-3:]
            ...
            if idx >= 0:
                ...
                if flag_append == 1:
                    ...
                    px.extend(data_planner.get("px"))
                    py.extend(data_planner.get("py"))
                    pz.extend(data_planner.get("pz"))
                    vx.extend(data_planner.get("vx"))
                    vy.extend(data_planner.get("vy"))
                    vz.extend(data_planner.get("vz"))
                    ax.extend(data_planner.get("ax"))
                    ay.extend(data_planner.get("ay"))
                    az.extend(data_planner.get("az"))

                    print(data_planner)
                time_list.append(idx*0.1)
            
            #print("data from mocap")
            #print(data_mocap)
