...
###########################################
    ...
    v_raw = np.zeros(3, dtype=float)
    v_window = np.zeros((width_window, 3), dtype=float)

    while owl.isOpen() and owl.property("initialized"):
        t0 = time.time()
        event = owl.nextEvent(1000)

        if idx % width_window == 0:
            idx_window = 0
        ...
        if event.type_id == Type.ERROR:
            ...
            if "rigids" in event:
                ...
                if r.cond > 0:
                    ...
                    #data = np.array([r.pose[0] / 1000.0, r.pose[1] / 1000.0, r.pose[2] / 1000.0, r.pose[3], r.pose[4], r.pose[5], r.pose[6]], dtype=float)

                    # mm to m
                    p1 = np.array([r.pose[0]/1000.0, r.pose[1]/1000.0, r.pose[2]/1000.0], dtype=float)
                    v_raw = (p1 - p0) / dt
                    v_window[idx_window, :] = v_raw
                    print(idx_window)
                    v_after = np.mean(v_window, axis=0)
                    print(v_after)
                    quat = np.array([r.pose[3], r.pose[4], r.pose[5], r.pose[6]], dtype=float)
                    p0 = p1
                    data = np.hstack((p1, v_after, quat))
