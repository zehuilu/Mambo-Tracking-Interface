...
# Remember to change the total time!!!!!!!!!!!!!!!!!
        ...
        posi_ini = np.array([[0.0], [0.6], [0.0], [0.0], [0.0], [0.0]])
        traj_ref = np.repeat(posi_ini, 20, axis=1)
        T = np.arange(0, 20, 1)
        while t_now < 10.00:
            if idx == 5:
                hover_flag = False
        ...
        #while t_now < 23.00:
            ...
            if not hover_flag:
                ...
                hf.LLC_test_yaw(idx, posi_now, point_ref_0, point_ref_1, yaw_now, yaw_des, Rot_Mat, P_now, velo_body, yaw_rate, vz_max, dt)
