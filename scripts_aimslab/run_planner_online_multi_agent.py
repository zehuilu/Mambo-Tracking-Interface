...
if __name__ == "__main__":
    ...
    while(iter_idx < (time_escape * frequency)):
        ...
        for idx in range(len(targets_position_qualisys)):
            heights_all.append(targets_position_qualisys[idx][-1])

        path_qualisys_all = MySimulator.path_index_all_to_qualisys(path_all_agents, heights_all)
        t1 = time.time()
        ...
        for idx in range(len(path_qualisys_all)):
            print("path_qualisys_all[idx]")
            print(path_qualisys_all[idx])
            time_queue_vec, position_traj, velocity_traj = discrete_path_to_time_traj(
                path_qualisys_all[idx], dt, velocity_ave, interp_kind='quadratic', ini_velocity_zero_flag=False)
