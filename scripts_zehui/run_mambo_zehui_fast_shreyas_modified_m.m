...
% quadrotor
v_max = 1 ; % SHREYAS CHANGED THIS TO 1
sensor_radius = 100 ;
...
% create planner
% SHREYAS CHANGED THIS: plot_zonotope_reach_set_flag and plot_zono_style
...
% SHREYAS ADDED THIS IF STATEMENT
if name_obstacles == 3 && v_max > 0.5
    disp('Modifying planner lookahead!')
    P.lookahead_scale = 0.75 ;
end
...
while iter_cur < max_iterations && t_cur < max_time && d_to_goal > W.goal_radius
    ...
    % SHREYAS CHANGED THIS
    N_Z = size(Z_NWU,2) ;
    d_to_goal = min(vecnorm(repmat(W.goal,1,N_Z) - Z_NWU(1:3,:))) ;
